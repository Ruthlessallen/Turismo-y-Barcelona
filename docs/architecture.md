...
| Bares/restaurantes (ciudad) — **fuente principal desde 2026-09-06** | [Open Data BCN — Cens de locals en planta baixa destinats a activitat econòmica](https://opendata-ajuntament.barcelona.cat/data/ca/dataset/cens-locals-planta-baixa-act-economica) — **verificado 2026-09-06** | Ciudad de Barcelona | CSV. Serie publicada: 2014, 2016, 2019, 2022 y **2024** (`241021_censcomercialbcn_opendata_2024_v5.csv`). **No hay edición de 2025 ni 2026.** La API CKAN deniega lectura anónima de este paquete; el catálogo se consulta por navegador | **Sustituye a OSM dentro de la ciudad**: OSM cuenta 7.430 locales de restauración y el censo 10.100, un 26% menos. Se detectó porque el cruce de terrazas contra OSM daba 93% de locales con terraza y doce barrios por encima del 100%. La cifra del censo coincide con la que declara el Ajuntament ("más de diez mil", guía 2025) y continúa su serie (9.359 en 2017). Filtrado por `Nom_Grup_Activitat` = restauración, quitando los 764 de alojamiento, que cuentan con los hoteles. **No trae tipo de cocina**: su clasificación más fina (`Nom_Activitat`) es tipo de local, así que el 67% de `tipo_cocina` vacío en OSM no se puede rellenar con esta fuente. Dos avisos del catálogo: desde 2022 solo recoge locales activos o pendientes, y el fichero de 2022 usa otra metodología y no es comparable. OSM se mantiene **fuera de la ciudad**, donde no hay alternativa |
| Terrazas (ciudad) | [Open Data BCN — Terrasses de restauració](https://opendata-ajuntament.barcelona.cat/data/ca/dataset/terrasses-restauracio) — **verificado 2026-09-06** | Ciudad de Barcelona | CSV, volcado único del 1 de julio de 2026, 6.942 licencias | **El fichero más limpio de la capa**: cero nulos en coordenadas, mesas, sillas, superficie, barrio y emplazamiento. Aporta 33.166 mesas, **127.482 sillas** y 83.745 m² de vía pública — la única medida de **capacidad** que tenemos en restauración, frente al resto de la capa, que solo cuenta locales. Contrastado con la cifra publicada (6.899 licencias en 2025, +220 concedidas ese año): encaja. **Descartado el 2026-09-10, el fichero está en la rama `scraps`.** No trae nombre de local ni identificador del censo, así que el vínculo hay que construirlo por dirección y proximidad, y el cruce no da: marca 5.661 locales cuando los reales son 6.227 —se pierde el 9% de las terrazas— y ensucia unos 380, entre portales compartidos (`potosí 2` marca **51 locales con una sola licencia**) e interiores de centro comercial. Se puede recuperar si algún día aparece un identificador común |
...
| Oferta anunciada en Airbnb | [Inside Airbnb](https://insideairbnb.com/) — **verificado 2026-08-27, descarga real probada** | Solo ciudad de Barcelona (confirmado: no publica el resto de municipios de la provincia por separado) | CSV directo en `data.insideairbnb.com/spain/catalonia/barcelona/<fecha>/visualisations/listings.csv` (+ `neighbourhoods.geojson`, útil como geometría de barrio). Snapshot verificado: 24/06/2026, 15.430 anuncios | **Hallazgo clave:** el campo `license` no es libre — Barcelona obliga a declarar el HUTB, y el **48% de los anuncios lo trae literal** (parseable con regex), lo que permite cruce **directo** por nº de licencia en vez de solo por dirección (ver `data-model.md` → `oferta_airbnb`, método `licencia_directa`). El resto no es automáticamente "sin licencia": una parte declara **exención** explícita (alquiler de temporada 31+ noches, hostel...), que se trata como categoría propia, no como oferta sin licencia. Coordenadas ofuscadas por privacidad: **hasta 150 m**, no 200 —cifra corregida el 2026-09-02 contra [insideairbnb.com/data-assumptions](https://insideairbnb.com/data-assumptions/), que la fija en 0-450 pies. El desplazamiento es aleatorio dentro de ese radio, así que no se puede acotar por anuncio. **Y los anuncios de un mismo edificio se anonimizan por separado**, de modo que se dispersan en vez de agruparse: por eso la detección de duplicados va por número de licencia y no por coordenada, que nunca coincidiría. El fallback por dirección sigue siendo una estimación, el match directo por licencia no. Ver referencia metodológica: [montera34/airbnb.barcelona](https://github.com/montera34/airbnb.barcelona)<br><br>**Fichero de reseñas** (`visualisations/reviews.csv`, verificado 2026-08-29): 1.033.523 filas con `listing_id` + fecha, 11.822 anuncios, serie 2010→2026. Es la única vía para estimar **duración de estancia**, que el snapshot de anuncios no trae: el hueco entre reseñas consecutivas de un anuncio acota por arriba la estancia intermedia. Sesgo conocido — incluye días vacíos entre huéspedes y fusiona estancias cuando alguien no reseña, así que **infla** los huecos y el % de estancias cortas que arroja es un suelo. Se descarta a propósito `data/reviews.csv.gz` (133 MB): añade nombre del huésped y texto del comentario, datos personales innecesarios para esto |
...
| Tarifa hotelera oficial (ADR) por categoría | [Portal de Dades del Ajuntament de Barcelona — *Average Daily Rate per Occupied Room by category*](https://portaldades.ajuntament.barcelona.cat/en/statistics/p77fcod53g) — **verificado 2026-09-02, descarga real** | Ciudad de Barcelona, cuatro categorías: 1-2 estrellas de oro **y estrellas de plata**, 3, 4 y 5 | CSV mensual desde enero 2013 hasta julio 2026 (163 meses × 4 categorías). Separador `;`, coma decimal, `-` donde no hay dato. Fuente última: INE, Encuesta de Ocupación Hotelera. **CC BY 4.0** | Es el **ancla externa del precio raspado**, la única fuente independiente que teníamos para validarlo: contrastado por categoría, los desvíos van de −3% a +12% en la dirección esperable (el ADR es precio cobrado tras descuentos; lo raspado es anunciado). Aporta además la **estacionalidad**, imprescindible porque el raspado es una sola ventana del 29-09 al 07-10: septiembre está un 16% sobre la media anual. La banda "estrellas de plata" cubre hostales y pensiones, que en el Registre figuran como "No aplica" y no tienen categoría propia en ninguna otra estadística. **El patrón estacional cambió tras la pandemia** —febrero cae un 13,7% y junio sube un 6,4% frente a 2013-2019— así que los factores se estiman solo desde 2022. La página es una aplicación JavaScript: no se puede descargar con `WebFetch`, hay que usar el botón de exportar o un navegador |
